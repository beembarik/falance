...
**Versi saat ini:** `0.3.0` · [Release notes](./CHANGELOG.md)

> **Status saat ini:** Milestone 0–9 telah selesai secara fungsional/operasional. Milestone 10 Slices 1–8 telah diimplementasikan dan divalidasi melalui pengujian production inti, termasuk Dashboard, Transaksi, Laporan, Akun/Keluarga, avatar Telegram viewer, create/edit/soft-void transaksi, administrasi keluarga, dan pesan invitation siap-share. Slice 9 telah menyiapkan kontrak category analytics secara lokal, tetapi belum mengaktifkan persistence atau UI kategori. Versi `0.3.0` menandai rangkaian perluasan Mini App tersebut. Validasi race lintas serverless instance dan production replay testing Milestone 8 tetap wajib sebelum public beta. Category summaries sengaja ditunda karena belum ada persisted `category` field pada `Transaction`. Sebelum deployment, `FALANCE_TELEGRAM_WEBHOOK_SECRET` harus diisi di Vercel dan secret yang sama harus dikonfigurasi pada Telegram `setWebhook`. AI parser, draft interaktif, receipt processing, approval, edit, pembatalan, validasi timezone, penanganan tanggal, category suggestion, dan description suggestion sudah tersedia.
