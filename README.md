...
| Telegram Mini App | Workspace mobile-first dengan Dashboard keluarga, App Shell, bottom navigation, screen Transaksi, Laporan, dan Akun/Keluarga. Endpoint report/account memvalidasi raw `initData`; report mendukung filter bulan/rentang maksimal 366 hari, maksimal 50 transaksi terbaru, comparison periode sebelumnya, dan tombol CSV/print/PDF hanya untuk `OWNER`/`ADMIN`. Screen Akun mencoba avatar viewer dari optional Telegram `photo_url` yang telah divalidasi, lalu beralih ke signed same-origin proxy melalui Bot API jika URL tidak tersedia atau gagal dimuat, tanpa mengekspos bot token, menyimpan URL, atau mengambil avatar anggota lain. Branding menggunakan warm green, lavender-purple, coral semantic accent, off-white background, dan mengabaikan warna hitam pekat pada background logo. CSV/PDF menggunakan signed HTTPS URLs melalui `Telegram.WebApp.downloadFile()` dan print menggunakan `Telegram.WebApp.openLink()`. Tambah Transaksi, edit/soft-void transaksi, administrasi keluarga, pesan undangan siap-share, dan selector kategori eksplisit sekarang tersedia melalui endpoint terotorisasi; category code Slice 9 telah dimigrasikan dan tervalidasi pada registry production. Kategori ditampilkan pada list/detail, sementara category summaries dan automatic AI category persistence tetap ditunda. | Milestone 10 Slices 1–8 production validated; Slice 9 production validated; Slice 10 implemented locally |
...
Milestone 8 sekarang **operationally ready untuk pengujian satu sampai dua keluarga** setelah production registry integrity check menghasilkan `healthy: true` dan `issues: []`. Validasi race lintas serverless instance dengan dua akun Telegram berbeda dan production replay testing tetap ditunda serta wajib diselesaikan sebelum public beta. Milestone 9 telah selesai secara operasional dan menyediakan read-only Telegram reports, authenticated Mini App reports, role-safe CSV export, print-friendly report, dan PDF server-side dengan optional password. M10 Slice 9 telah mempersist category code stabil, memigrasikan row legacy secara aman, dan tervalidasi pada registry production dengan `healthy: true` serta `issues: []`; category summaries tetap ditunda. **Milestone 10 — Telegram Mini App Expansion (P1)** sedang berjalan. Slice 1–8 telah diimplementasikan pada workspace Mini App, termasuk Dashboard, Transaksi, Laporan, Akun/Keluarga, readability branding, avatar Telegram viewer, tambah/edit transaksi, soft-void, dan administrasi keluarga terbatas. Slice 10 sedang diimplementasikan untuk selector kategori eksplisit pada create/edit transaction, menampilkan label category pada list/detail, tetap memakai validasi `initData` dan resolusi family server-side, serta tidak mempersist `categorySuggestion` AI secara otomatis. Anggaran, category summaries, payment method, AI financial insight, dan Mini App receipt scanning tetap deferred sampai kontrak dan service boundary masing-masing siap. **Milestone 11 — AI Usage, Quota, and Provider Reliability (P2)**, **Milestone 12 — Supabase Migration (P2)**, dan **Milestone 13 — Monetization and Expansion (P3)** tetap berada di roadmap. Urutan ini mempertahankan fokus pada satu sampai dua keluarga terlebih dahulu, menempatkan hardening sebelum public beta, dan menunda usage tracking, quota, monetisasi, serta scale-out sampai kebutuhan operasionalnya terbukti.
