...
**Versi saat ini:** `0.7.8` · [Release notes](./CHANGELOG.md)

> **Status saat ini:** Milestone 0–9 telah selesai secara fungsional/operasional. Milestone 10 Slices 1–17 telah diimplementasikan dan divalidasi melalui pengujian production inti, termasuk Dashboard, Transaksi, Laporan, Akun/Keluarga, avatar Telegram viewer, create/edit/soft-void transaksi, administrasi keluarga, category analytics, cash-flow analytics, category drill-down, branded report preview/export, transparent Falancé symbol branding, semantic report card colors, dan command `/help`. Slice 18 hardening security dan konfigurasi provider AI text/vision terpisah telah diimplementasikan lokal dan menunggu validasi production. M11 Slice 1 menambahkan tracking dan quota AI text secara privacy-safe di worksheet pusat terpisah, telah diimplementasikan lokal dan menunggu quality-gate serta validasi production. Validasi race lintas serverless instance, production replay testing Milestone 8, serta checklist pre-public-beta tetap wajib sebelum public beta. Sebelum deployment, `FALANCE_TELEGRAM_WEBHOOK_SECRET` harus diisi di Vercel dan secret yang sama harus dikonfigurasi pada Telegram `setWebhook`. AI parser, draft interaktif, receipt processing, approval, edit, pembatalan, validasi timezone, penanganan tanggal, category suggestion, dan description suggestion sudah tersedia.
...
| AI text parser | Pesan natural-language dari anggota aktif diekstrak menjadi draft transaksi tervalidasi. Bot menampilkan rekap dengan tombol `Ya, simpan`, `Edit`, dan `Batalkan`; `Ya, simpan`/`Kirim draft` menyimpan melalui service deterministik, sedangkan `Edit` membuka format `/editdraft`. Tanggal transaksi biasa tidak boleh melewati tanggal hari ini pada zona waktu bisnis yang dikonfigurasi. Jika transaksi aktual tidak menyebut tanggal, draft menggunakan hari ini dan menampilkannya sebagai `(diasumsikan hari ini)`. Draft dapat menampilkan saran kategori terkontrol dan saran deskripsi opsional untuk ditinjau; bahasa perencanaan tidak disimpan sebagai transaksi aktual. Provider text dapat dikonfigurasi terpisah melalui `FALANCE_AI_TEXT_API_BASE`, `FALANCE_AI_TEXT_API_KEY`, dan `FALANCE_AI_TEXT_MODEL`, dengan fallback kompatibilitas ke variable shared lama. Fallback optional text memakai `FALANCE_AI_TEXT_FALLBACK_API_BASE`, `FALANCE_AI_TEXT_FALLBACK_API_KEY`, dan `FALANCE_AI_TEXT_FALLBACK_MODEL`; hanya satu fallback attempt untuk kegagalan transient. Degraded mode memetakan quota exhaustion dan kegagalan provider ke pesan Bahasa Indonesia yang aman tanpa detail internal. Setiap claim natural-language memakai cooldown, rolling-window quota, dan in-flight lease durable per family/user pada worksheet `AI Text Usage`; yang dihitung adalah attempt, termasuk provider failure, untuk mencegah retry storm. Worksheet hanya menyimpan metadata operasional dan tidak menyimpan prompt, response, token, atau receipt content. | Tersedia; Milestone 6 selesai; M11 Slices 1–4 implemented locally |
| Receipt photo intake | Photo receipt dari anggota aktif divalidasi, diunduh melalui `getFile` secara server-side, lalu diarahkan ke receipt parser vision. Gambar tidak disimpan ke Google Sheets dan transaksi tidak dibuat sebelum approval draft. Provider vision dapat dikonfigurasi terpisah melalui `FALANCE_AI_VISION_API_BASE`, `FALANCE_AI_VISION_API_KEY`, dan `FALANCE_AI_VISION_MODEL`; variable shared lama tetap didukung sebagai fallback. Optional fallback vision memakai prefix `FALANCE_AI_VISION_FALLBACK_*` dan mengikuti satu attempt transient yang sama. Degraded mode receipt memberi alternatif input manual saat service tidak tersedia atau response tidak dapat divalidasi. Production validation berhasil dengan OpenRouter menggunakan model text dan vision yang terpisah. | Tersedia; Milestone 7 selesai; M11 Slices 3–4 implemented locally |
| Isolasi keluarga | `family_id` selalu ditentukan server dari membership aktif atau invitation yang telah divalidasi. | Tersedia |
| Registry Google Sheets | Satu registry pusat menggunakan tiga belas worksheet: `Settings`, `Families`, `Members`, `Invitations`, `Pending Family Creations`, `Pending Confirmations`, `Pending Transaction Drafts`, `Audit Log`, `Transactions`, `Processed Telegram Updates`, `AI Vision Usage`, `AI Text Usage`, dan `Draft Approval Claims`. Worksheet claim menyimpan state durable untuk replay/update processing, AI vision usage, dan approval recovery. | Tersedia |
...
| Telegram Mini App | Workspace mobile-first dengan Dashboard keluarga, App Shell, bottom navigation, screen Transaksi, Laporan, dan Akun/Keluarga. Endpoint report/account memvalidasi raw `initData`; report mendukung filter bulan/rentang maksimal 366 hari, maksimal 50 transaksi terbaru, comparison periode sebelumnya, category summaries server-side, provenance `dicatat oleh`, dan tombol CSV/print/PDF hanya untuk `OWNER`/`ADMIN`. Beranda berfokus pada family financial snapshot dan aktivitas terbaru, sedangkan Laporan menjadi permukaan analytics. Screen Akun mencoba avatar viewer dari optional Telegram `photo_url` yang telah divalidasi, lalu beralih ke signed same-origin proxy melalui Bot API jika URL tidak tersedia atau gagal dimuat, tanpa mengekspos bot token, menyimpan URL, atau mengambil avatar anggota lain. Branding menggunakan warm green, lavender-purple, coral semantic accent, off-white background, dan mengabaikan warna hitam pekat pada background logo. CSV/PDF menggunakan signed HTTPS URLs melalui `Telegram.WebApp.downloadFile()` dan print menggunakan `Telegram.WebApp.openLink()`. Tambah Transaksi, edit/soft-void transaksi, administrasi keluarga, pesan undangan siap-share, selector kategori eksplisit, dan category analytics read-only sekarang tersedia melalui endpoint terotorisasi; category code Slice 9 telah dimigrasikan dan tervalidasi pada registry production. Category summaries tetap per-currency dan bukan budget, savings, atau AI insight. | Milestone 10 Slices 1–14 production validated |
...
| `AI Text Usage` | Cooldown, rolling-window quota, dan in-flight lease untuk natural-language text per active user/family; hanya metadata operasional, tanpa prompt, response, token, atau receipt content. |
...
- AI usage tracking hanya menyimpan metadata operasional untuk quota per server-resolved family/user/workload; prompt, response, token count, receipt content, API key, dan raw identity tidak dicatat atau dikirim ke browser/Telegram.
...
| `FALANCE_AI_API_BASE` | Endpoint provider AI OpenAI-compatible shared legacy; dipakai sebagai fallback jika konfigurasi dedicated tidak diisi. | Tidak |
| `FALANCE_AI_API_KEY` | API key provider AI shared legacy; server-only dan tidak boleh dikirim ke client atau Telegram. | Tidak |
| `FALANCE_AI_MODEL` | Model AI text parser shared legacy; dipakai sebagai fallback text dan default `gpt-5-mini`. | Tidak |
| `FALANCE_AI_TEXT_API_BASE` | Endpoint dedicated provider text OpenAI-compatible; mengoverride `FALANCE_AI_API_BASE` untuk text parser. | Tidak |
| `FALANCE_AI_TEXT_API_KEY` | API key dedicated provider text; server-only. | Tidak |
| `FALANCE_AI_TEXT_MODEL` | Model dedicated text parser; mengoverride `FALANCE_AI_MODEL`. | Tidak |
| `FALANCE_AI_TEXT_FALLBACK_API_BASE` | Endpoint server-only fallback text; fallback hanya aktif jika base, key, dan model lengkap. | Tidak |
| `FALANCE_AI_TEXT_FALLBACK_API_KEY` | API key server-only fallback text. | Tidak |
| `FALANCE_AI_TEXT_FALLBACK_MODEL` | Model server-only fallback text. | Tidak |
| `FALANCE_AI_TEXT_COOLDOWN_SECONDS` | Cooldown AI text per active user/family; default `5` detik. | Tidak |
| `FALANCE_AI_TEXT_WINDOW_SECONDS` | Rolling window quota AI text; default `3600` detik. | Tidak |
| `FALANCE_AI_TEXT_MAX_REQUESTS` | Maksimum claim AI text per active user/family/window; default `30`. Claim dihitung sebagai attempt, termasuk provider failure. | Tidak |
| `FALANCE_AI_TEXT_LEASE_SECONDS` | Lease claim AI text yang sedang berjalan; default `60` detik. | Tidak |
| `FALANCE_AI_VISION_API_BASE` | Endpoint dedicated provider vision OpenAI-compatible; mengoverride `FALANCE_AI_API_BASE` untuk receipt parser. | Tidak |
| `FALANCE_AI_VISION_API_KEY` | API key dedicated provider vision; server-only. | Tidak |
| `FALANCE_AI_VISION_MODEL` | Model vision-capable dedicated untuk receipt parser; wajib jika receipt parsing diaktifkan. | Tidak |
| `FALANCE_AI_VISION_FALLBACK_API_BASE` | Endpoint server-only fallback vision; fallback hanya aktif jika base, key, dan model lengkap. | Tidak |
| `FALANCE_AI_VISION_FALLBACK_API_KEY` | API key server-only fallback vision. | Tidak |
| `FALANCE_AI_VISION_FALLBACK_MODEL` | Model server-only fallback vision. | Tidak |
...
Milestone 8 sekarang **operationally ready untuk pengujian satu sampai dua keluarga** setelah production registry integrity check menghasilkan `healthy: true` dan `issues: []`. Validasi race lintas serverless instance dengan dua akun Telegram berbeda dan production replay testing tetap ditunda serta wajib diselesaikan sebelum public beta. Milestone 9 telah selesai secara operasional dan menyediakan read-only Telegram reports, authenticated Mini App reports, role-safe CSV export, print-friendly report, dan PDF server-side dengan optional password. M10 Slices 9–14 telah diproduksi dan divalidasi, termasuk category code stabil, migration legacy, assignment kategori manual, category analytics read-only per currency, refinement Beranda/Laporan, provenance pencatat transaksi, cash-flow analytics, serta filter dan drill-down kategori read-only. **Milestone 10 — Telegram Mini App Expansion (P1)** sedang berjalan. M10 Slice 15 sedang mengonsolidasikan export menjadi branded report preview dengan satu entry point pada card ringkasan dan favicon symbol transparan. Account/transfer/savings, payment method, budget, AI financial insight, dan Mini App receipt scanning tetap deferred sampai kontrak dan service boundary masing-masing siap. Versi Falancé dinaikkan menjadi 0.6.0 setelah validasi production Slice 14; setiap perubahan lintas slice berikutnya akan dievaluasi untuk release version berikutnya.
 **Milestone 11 — AI Usage, Quota, and Provider Reliability (P2)**, **Milestone 12 — Supabase Migration (P2)**, dan **Milestone 13 — Monetization and Expansion (P3)** tetap berada di roadmap. Urutan ini mempertahankan fokus pada satu sampai dua keluarga terlebih dahulu, menempatkan hardening sebelum public beta, dan menunda usage tracking, quota, monetisasi, serta scale-out sampai kebutuhan operasionalnya terbukti.
