...
| Telegram Mini App | UI read-only mobile-first, endpoint report yang memvalidasi raw `initData`, launcher `/reportapp`, filter bulan/rentang tanggal maksimal 366 hari, daftar maksimal 50 transaksi terbaru, serta tombol CSV hanya untuk `OWNER`/`ADMIN`. | Milestone 9 Slice 2–3 tersedia |
...
Pada tahap laporan, semua role aktif dapat melihat report melalui Telegram dan Mini App. `OWNER` dan `ADMIN` dapat mengunduh CSV melalui Mini App; print dan PDF masih direncanakan. `MEMBER` tidak dapat melakukan export, dan pembatasan tersebut ditegakkan ulang pada server.
