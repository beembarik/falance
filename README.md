...
Milestone 8 sekarang **operationally ready untuk pengujian satu sampai dua keluarga** setelah production registry integrity check menghasilkan `healthy: true` dan `issues: []`. Validasi race lintas serverless instance dengan dua akun Telegram berbeda dan production replay testing tetap ditunda serta wajib diselesaikan sebelum public beta. Milestone 9 telah selesai secara operasional dan menyediakan read-only Telegram reports, authenticated Mini App reports, role-safe CSV export, print-friendly report, dan PDF server-side dengan optional password. M10 Slices 9–10 telah mempersist category code stabil, memigrasikan row legacy secara aman, menyediakan assignment kategori manual pada Mini App, dan tervalidasi pada registry/production Mini App. **Milestone 10 — Telegram Mini App Expansion (P1)** sedang berjalan. Slice 11 menambahkan category summaries server-side dan visualisasi pengeluaran per kategori pada Dashboard, dengan pemisahan currency dan tanpa budget atau AI insight. Payment method, budget, AI financial insight, dan Mini App receipt scanning tetap deferred sampai kontrak dan service boundary masing-masing siap. **Milestone 11 — AI Usage, Quota, and Provider Reliability (P2)**, **Milestone 12 — Supabase Migration (P2)**, dan **Milestone 13 — Monetization and Expansion (P3)** tetap berada di roadmap. Urutan ini mempertahankan fokus pada satu sampai dua keluarga terlebih dahulu, menempatkan hardening sebelum public beta, dan menunda usage tracking, quota, monetisasi, serta scale-out sampai kebutuhan operasionalnya terbukti.
