...
**Versi saat ini:** `0.7.1` · [Release notes](./CHANGELOG.md)

> **Status saat ini:** Milestone 0–9 telah selesai secara fungsional/operasional. Milestone 10 Slices 1–15 telah diimplementasikan dan divalidasi melalui pengujian production inti, termasuk Dashboard, Transaksi, Laporan, Akun/Keluarga, avatar Telegram viewer, create/edit/soft-void transaksi, administrasi keluarga, category analytics, cash-flow analytics, category drill-down, branded report preview/export, dan transparent Falancé symbol branding. Slice 16 semantic report card colors telah diimplementasikan secara lokal dan menunggu validasi production. Validasi race lintas serverless instance dan production replay testing Milestone 8 tetap wajib sebelum public beta. Sebelum deployment, `FALANCE_TELEGRAM_WEBHOOK_SECRET` harus diisi di Vercel dan secret yang sama harus dikonfigurasi pada Telegram `setWebhook`. AI parser, draft interaktif, receipt processing, approval, edit, pembatalan, validasi timezone, penanganan tanggal, category suggestion, dan description suggestion sudah tersedia.
