...
| Telegram Mini App | UI read-only mobile-first, endpoint report yang memvalidasi raw `initData`, launcher `/reportapp`, filter bulan/rentang tanggal maksimal 366 hari, daftar maksimal 50 transaksi terbaru, serta tombol CSV dan tampilan cetak hanya untuk `OWNER`/`ADMIN`. | Milestone 9 Slice 2–4 tersedia |
...
| Print-friendly report | `OWNER` dan `ADMIN` dapat membuka HTML report family-scoped dari Mini App, lalu mencetak atau memilih Save as PDF melalui browser. Output menggunakan HTML escaping dan print stylesheet; PDF server-side belum tersedia. | Milestone 9 Slice 4 tersedia |
...
Pada tahap laporan, semua role aktif dapat melihat report melalui Telegram dan Mini App. `OWNER` dan `ADMIN` dapat mengunduh CSV dan membuka tampilan cetak melalui Mini App; PDF server-side dan category summaries masih direncanakan. `MEMBER` tidak dapat melakukan export, dan pembatasan tersebut ditegakkan ulang pada server.
