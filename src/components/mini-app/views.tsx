...
export function CashFlowSection({ points }: { points: ReportResponse["report"]["cashFlow"] }) {
...
export function CashFlowChart({ currency, points }: { currency: string; points: ReportResponse["report"]["cashFlow"] }) {
...
export function CashFlowBar({ label, value, total, color, currency }: { label: string; value: string; total: bigint; color: string; currency: string }) {
...
export function CategoryExpenseSection({ summaries, selected, onSelect }: { summaries: ReportResponse["report"]["categorySummaries"]; selected: { category: string; currency: string } | null; onSelect: (value: { category: string; currency: string } | null) => void }) {
...
export function CategoryExpenseChart({ currency, summaries, selected, onSelect }: { currency: string; summaries: ReportResponse["report"]["categorySummaries"]; selected: { category: string; currency: string } | null; onSelect: (value: { category: string; currency: string } | null) => void }) {
...
export function AddTransactionForm({ initData, transaction, onClose, onSaved }: { initData: string; transaction?: ReportResponse["report"]["transactions"][number]; onClose: () => void; onSaved: () => void }) {
...
export function AccountView({ data, onRetry, onFamilyAction, familyAction, familyActionError, invitation, deactivateConfirmation }: {
  ...
  return <>
    <section className="overflow-hidden rounded-2xl bg-[var(--brand-purple-600)] text-white shadow-[0_8px_24px_rgba(38,122,90,0.16)]"><div className="p-5"><div className="flex items-center gap-4"><UserAvatar key={`${data.viewer.avatarUrl ?? ""}:${data.viewer.avatarFallbackUrl ?? ""}`} name={data.viewer.name} avatarUrl={data.viewer.avatarUrl} avatarFallbackUrl={data.viewer.avatarFallbackUrl} size="large" /><div className="min-w-0"><p className="truncate text-xl font-bold">{data.viewer.name}</p><p className="mt-1 text-sm text-emerald-100">{roleLabel}{data.viewer.username ? ` · @${data.viewer.username}` : ""}</p></div></div></div><div className="border-t border-white/15 bg-[var(--brand-coral-500)] px-5 py-4"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-100">Konteks akses</p><p className="mt-1 text-sm leading-6 text-white/90">{permissionText}</p></div></section>
...
export function UserAvatar({ name, avatarUrl, avatarFallbackUrl, size = "small" }: { name: string; avatarUrl?: string | null; avatarFallbackUrl?: string | null; size?: "small" | "large" }) {
...
export function TransactionsView({ data, filter, onFilterChange, onSelectTransaction }: { data: ReportResponse; filter: TransactionFilter; onFilterChange: (value: TransactionFilter) => void; onSelectTransaction: (transaction: ReportResponse["report"]["transactions"][number]) => void }) {
...
export function TransactionDetail({ transaction, onClose, onEdit, onRequestVoid, voidConfirmation, transactionAction, transactionActionError, onConfirmVoid, onCancelVoid }: {
...
export function ComparisonSummary({ current, previous }: { current: ReportResponse; previous: ReportResponse }) {
...
export function ComparisonMetric({ label, current, previous, currency, positiveIsGood }: { label: string; current: string; previous: string; currency: string; positiveIsGood: boolean }) {
...
export function ReportsView({ data, comparison, comparisonLoading, month, startDate, endDate, loading, onMonthChange, onStartDateChange, onEndDateChange, onLoad, printing, printError, onPrint, onCsv, csvDownloading, csvDownloadSupported, categoryFilter, onCategoryFilterChange }: {
  data: ReportResponse;
...
export function MetricCard({ summary }: { summary: ReportResponse["report"]["currencies"][number] }) {
...
export function TransactionRow({ transaction }: { transaction: ReportResponse["report"]["transactions"][number] }) {
...
export function PlaceholderView({ title, description, actionLabel, onAction }: { title: string; description: string; actionLabel: string; onAction: () => void }) {
...
export function BottomNavigation({ activeNav, onSelect, onAddTransaction }: { activeNav: NavKey; onSelect: (key: NavKey) => void; onAddTransaction: () => void }) {
  return (
...
export function NavButton({ item, active, onClick }: { item: { key: NavKey; label: string; color: "green" | "purple" | "coral" }; active: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
