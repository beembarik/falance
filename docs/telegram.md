...
Milestone 9 now provides authorized Telegram summaries, authenticated Mini App views, and role-safe CSV export as the first implemented report channels. All active members may view reports through Telegram and the Mini App, but only `OWNER` and `ADMIN` may request or receive export artifacts. CSV export is currently initiated from the Mini App rather than a Telegram `/export` command. The export API validates raw `initData`, resolves the requester’s active membership and `family_id` server-side, checks the role before generating CSV, and returns only the requester’s family data. `MEMBER` requests are rejected even when sent directly to the endpoint. Future print and PDF endpoints must preserve the same boundary and must never expose the Google Spreadsheet directly.
