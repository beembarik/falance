...
| `/reportapp` | Sends a HTTPS `web_app` button that opens the Mini App. The browser sends raw Telegram `initData` to the server; the server validates its HMAC and `auth_date`, extracts the Telegram user ID, and resolves membership/family server-side. The Mini App supports a calendar month or a date range up to 366 days, returns up to 50 newest active transaction details for viewing, and shows `Unduh CSV`, `Tampilan cetak`, and `Unduh PDF` only to `OWNER` and `ADMIN`. Each request is authorized again server-side and uses all matching active transaction details. Export and print controls submit a native `POST` form to a new browser context rather than relying on client-side Blob URLs; this is deliberate because Telegram WebView download and popup behavior is not equivalent to a desktop browser. The print response is HTML with an in-browser print stylesheet. The PDF response is generated server-side; an optional password is sent only in the secure request body, used ephemerally, and never included in a URL, log, Sheets row, or response message. Passwords must be 8–127 UTF-8 bytes. There is no client-controlled `family_id` and no direct spreadsheet link. |
...
Milestone 9 now provides authorized Telegram summaries, authenticated Mini App views, role-safe CSV export, role-safe print-friendly HTML reports, and role-safe server-side PDF export. All active members may view reports through Telegram and the Mini App, but only `OWNER` and `ADMIN` may request or receive export artifacts. CSV, print, and PDF are initiated from the Mini App rather than a Telegram `/export` command. The Mini App uses direct native form POST navigation for these binary/HTML responses so the WebView can handle the response as a normal browser navigation or download. Each endpoint accepts JSON for programmatic tests and form POST for WebView delivery, validates raw `initData`, resolves the requester’s active membership and `family_id` server-side, checks the role before generating output, and returns only the requester’s family data. `MEMBER` requests are rejected even when sent directly to an endpoint. Category summaries remain deferred until a persisted transaction category schema is defined.

If a user requests a password-protected PDF, the password is collected through the Mini App password input, sent in the POST body, used only during server-side PDF generation, and excluded from Telegram logs, URLs, Sheets, analytics, and persistent storage. The client clears the password after a successful download, and the server never echoes it in a response message.
