...
| `/reportapp` | Sends a HTTPS `web_app` button that opens the Mini App. The browser sends raw Telegram `initData` to the server; the server validates its HMAC and `auth_date`, extracts the Telegram user ID, and resolves membership/family server-side. The Mini App supports a calendar month or a date range up to 366 days, returns up to 50 newest active transaction details for viewing, and shows `Unduh CSV` and `Tampilan cetak` only to `OWNER` and `ADMIN`. Both requests are authorized again server-side and use all matching active transaction details. The print response is an HTML document with an in-browser print stylesheet; server-side encrypted PDF remains a future slice. There is no client-controlled `family_id` and no direct spreadsheet link. |
...
Milestone 9 now provides authorized Telegram summaries, authenticated Mini App views, role-safe CSV export, and role-safe print-friendly HTML reports. All active members may view reports through Telegram and the Mini App, but only `OWNER` and `ADMIN` may request or receive export artifacts. CSV and print are currently initiated from the Mini App rather than a Telegram `/export` command. Each endpoint validates raw `initData`, resolves the requester’s active membership and `family_id` server-side, checks the role before generating output, and returns only the requester’s family data. `MEMBER` requests are rejected even when sent directly to an endpoint. Future PDF endpoints must preserve the same boundary and must never expose the Google Spreadsheet directly.
