...
| `/reportapp` | Sends a HTTPS `web_app` button that opens the Mini App. The browser sends raw Telegram `initData` to the server; the server validates its HMAC and `auth_date`, extracts the Telegram user ID, and resolves membership/family server-side. The Mini App supports a calendar month or a date range up to 366 days, returns up to 50 newest active transaction details for viewing, and shows `Unduh CSV`, `Tampilan cetak`, and `Unduh PDF` only to `OWNER` and `ADMIN`. Each request is authorized again server-side and uses all matching active transaction details. The print response is HTML with an in-browser print stylesheet. The PDF response is generated server-side; an optional password is sent only in the secure request body, used ephemerally, and never included in a URL, log, Sheets row, or response message. Passwords must be 8–127 UTF-8 bytes. There is no client-controlled `family_id` and no direct spreadsheet link. |
