...
Status: IN PROGRESS — SLICES 1–4 IMPLEMENTED LOCALLY; VALIDATION PRODUCTION PENDING
...
The fallback remains inside the same parser invocation and the same AI quota claim. Its response uses the existing draft normalization and schema validation, and no fallback path can write a transaction directly. Provider loops, unbounded retry, and automatic persistence remain deferred.
...
Status: LAUNCH-SAFE BASELINE COMPLETE — Supabase migration deferred after public launch.

M12 tidak dibagi menjadi slice. Untuk menghindari ambiguitas, milestone ini sekarang memiliki dua batas yang berbeda: **launch-safe baseline** dan **future migration completion**. Launch-safe baseline berarti Production tetap menggunakan Google Sheets, Supabase-primary tidak aktif di Production, shadow-read tidak aktif di Production, dan seluruh perubahan Supabase bersifat non-blocking terhadap operasi pengguna. Future migration completion baru tercapai setelah observation, compatibility validation, approved cutover, rollback simulation, dan deprecation plan selesai. Prosedur operasional lengkap tersedia di [`docs/supabase-migration-runbook.md`](supabase-migration-runbook.md).

The implementation layer keeps `google-sheets` as the default `FALANCE_PERSISTENCE_BACKEND`. `npm run rehearse:migration` validates a sanitized thirteen-worksheet snapshot using schema rules, foreign-reference checks, row counts, and deterministic canonical digests. Migration `0001_initial_schema.sql` defines the PostgreSQL tables and deny-by-default RLS; `0002_atomic_operations.sql` defines server-side atomic claims for update IDs, draft approvals, invitations, and AI usage. The read adapter now matches Google Sheets filtering semantics, including valid PostgREST `eq`/`in` filter syntax, and `SupabaseFamilyRepository` implements the repository contract behind an injected server-side write client. A live write validation on the dedicated Supabase test project passed snapshot import projection, family/member/transaction parity, update ID claims, draft approval claims and completion, one-time invitation consumption, AI text/vision quota claims, lease/counter/completion behavior, and cleanup with zero fixture rows remaining. The validation found and fixed a dynamic-SQL existence check in `claim_ai_usage`: the function now checks `usage_row is null` instead of relying on `IF NOT FOUND`. The reproducible SQL scenario is stored at `scripts/live-supabase-write-validation.sql`. None of this connects production or performs a cutover.

Authenticated Preview acceptance was subsequently completed against the guarded Supabase-primary adapter. The Mini App loaded account and report data with HTTP 200, then successfully created a test transaction with HTTP 201, updated it with HTTP 200, voided it with HTTP 200, and reloaded the report successfully. The test database was populated manually from a sanitized CSV copy because service-role import access was not available in the sandbox; the test fixture was removed after acceptance. This validates the Preview path only and does not replace the planned automated import, observation period, Production compatibility validation, or approved cutover procedure.

Supabase is the planned future storage implementation, not an immediate replacement. Google Sheets remains the active adapter until migration readiness is demonstrated for the current one-to-two-family operating scope. The project may proceed to public launch on the Google Sheets-primary path; public launch does not imply a Supabase cutover or waive the remaining migration controls.

### M12 launch boundary and post-launch work

The launch decision is intentionally conservative: Falancé launches with Google Sheets as the sole Production source of truth, with `FALANCE_PERSISTENCE_BACKEND=google-sheets`, no Production Supabase service-role credentials, and no Production shadow-read flag. The Supabase schema, RLS, atomic RPCs, repository seam, Preview acceptance, local rehearsal, and Google Sheets quota mitigation remain available as a controlled migration foundation. The latest Preview read-only observation showed successful Mini App account/report responses with no observed `5xx`, `429`, mismatch, or secondary-failure log; because successful comparisons are intentionally silent, this is evidence of stability rather than a complete quantitative parity proof.

The following work remains explicitly post-launch and must not be compressed into an unsafe release-day cutover: a measurable Preview shadow-read observation window, production compatibility validation for Telegram commands/drafts/receipts/reports/audit behavior, a controlled backup and write-freeze procedure, idempotent full-state import including operational worksheets, final delta reconciliation, rollback simulation including post-switch writes, an explicitly approved Production cutover, and Google Sheets deprecation. Until those gates are approved, any Supabase environment is test/Preview only.
