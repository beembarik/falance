...
`OWNER` and `ADMIN` may request CSV, print-friendly HTML, and server-side PDF reports. PDF password protection is optional and implemented through an ephemeral request-body password. `MEMBER` may view authorized reports through Telegram and the authenticated Mini App but must be denied export requests before report generation or artifact creation. This is a role authorization rule, not a client-interface rule; the backend must enforce it on every export endpoint.
